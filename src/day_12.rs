...
pub fn calculate_price(use_edges_instead_of_perimeter: bool) -> i32 {
    ...
    for y in 0..map.height as i32 {
        for x in 0..map.width as i32 {
            ...
            let cost = area
                * if use_edges_instead_of_perimeter {
                    corner_count
                } else {
                    perimeter
                };
...
impl Map {
    pub fn get_field(&self, location: &(i32, i32)) -> Option<char> {
        if location.1 >= 0
            && location.1 < self.field.len() as i32
            && location.0 >= 0
            && location.0 < self.field[0].len() as i32
        {
...
fn traverse_area(
    ...
    area: &mut i32,
    perimeter: &mut i32,
    corner_count: &mut i32,
    known_fields: &mut HashSet<(i32, i32)>,
) {
    ...
    *area += 1;
    ...
    let n = map
        .get_field(&add(&current_location, &(0, -1)))
        .is_some_and(|c| c == char);
    let e = map
        .get_field(&add(&current_location, &(1, 0)))
        .is_some_and(|c| c == char);
    let s = map
        .get_field(&add(&current_location, &(0, 1)))
        .is_some_and(|c| c == char);
    let w = map
        ...
        .is_some_and(|c| c == char);
    let ne = map
        .get_field(&add(&current_location, &(1, -1)))
        .is_some_and(|c| c == char);
    let se = map
        .get_field(&add(&current_location, &(1, 1)))
        .is_some_and(|c| c == char);
