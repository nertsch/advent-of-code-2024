...
fn count_trail_heads(
    expected_height: u8,
    position: Vector2<i32>,
    map_size: &Vector2<i32>,
    map: &Vec<Vec<u8>>,
    known_heads: &mut HashSet<Vector2<i32>>,
    count_unique_heads: bool,
) -> usize {
    let mut head_count = 0;
    if map[position.y as usize][position.x as usize] == expected_height {
        if expected_height == 9 {
            if known_heads.insert(position) || count_unique_heads {
                head_count += 1;
            }
        } else {
            let steps: [nalgebra::Vector2<i32>; 4] =
                [vector![-1, 0], vector![1, 0], vector![0, -1], vector![0, 1]];
            for step in steps {
                let next_step = position + step;
                if next_step.x >= 0
                    && next_step.x < map_size.x
                    && next_step.y >= 0
                    && next_step.y < map_size.y
                {
                    head_count += count_trail_heads(
                        expected_height + 1,
                        next_step,
                        map_size,
                        map,
