...
pub fn part_b() -> usize{
    ...
    map.process_antenna_tuples(
        |a1,a2|{
            let antenna_distance = a2 - a1;
            let paths : [(&Vector2<i32>, Vector2<i32>);2] = [(a1,-antenna_distance),(a2,antenna_distance)];
...
impl Map {
    pub fn read_from_file() -> Map {
        ...
        let mut antenna_positions_by_antenna_type: HashMap<char,Vec<Vector2<i32>>> = HashMap::new();
    ...
    pub fn process_antenna_tuples(&self, mut process_tuple : impl FnMut(&Vector2<i32>, &Vector2<i32>)) {
        for antenna_positions in self.antenna_positions_by_antenna_type.values(){
            if antenna_positions.len() <= 1 { continue;}
