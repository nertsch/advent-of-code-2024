...
pub fn get_cheat_count(min_savings: usize, max_cheat_time: usize) -> i32 {
    ...
    for start in 0..path.len(){
        for end in (start + min_savings)..path.len() {
            let manhattan_distance = path.get(start).unwrap().0.abs_diff(path.get(end).unwrap().0) + path.get(start).unwrap().1.abs_diff(path.get(end).unwrap().1);
            if manhattan_distance <= max_cheat_time && manhattan_distance <= end - start - min_savings {
                number_of_cheats +=1 ;
            }
        }
    }
